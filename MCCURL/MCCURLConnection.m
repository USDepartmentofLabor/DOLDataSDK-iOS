...
//
//  Updated by Mike Pulsifer for ARC support on 11/26/13
...
+ (void)setOnRequest:(void(^)(MCCURLConnection *))callback {  __onRequest = [callback copy]; }
+ (void(^)(MCCURLConnection *))onRequest { return __onRequest; }

static NSOperationQueue *__queue = nil;
+ (void)setQueue:(NSOperationQueue *)queue {  __queue = queue ; }
...
+ (id)connection { return [[[self class]alloc]init]; }
...
+ (id)context {
  MCCURLConnection *context = [[self alloc]init];
...
- (id)connection {
  ...
  MCCURLConnection *connection = [[[self class]alloc]init];
...
- (void)enqueueRequest:(NSURLRequest *)request {
  ...
  [queue addOperationWithBlock:^{
    ...
    dispatch_sync(__synchronized, ^{
      ...
      self.uconnection = [[NSURLConnection alloc]initWithRequest:request delegate:self startImmediately:NO];
